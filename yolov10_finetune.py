...
class PretrainNet(nn.Module):
    def __init__(self, backbone):
        ...
        self.stage4 = deepcopy(nn.Sequential(backbone[5], backbone[6]))  # 5:6
        self.stage5 = deepcopy(nn.Sequential(*[backbone[i] for i in range(7, 11)]))  # 6:10
...
state_dict = torch.load("pretrain_net_v10.pth", map_location="cpu")
...
stage4_yolo = nn.Sequential(backbone[5], backbone[6])
stage5_yolo = nn.Sequential(*[backbone[i] for i in range(7, 11)])
...
# Train the model
results = yolo.train(data="OTO160K.yaml", epochs=50, imgsz=640, freeze=11, optimizer="auto", amp=True,
